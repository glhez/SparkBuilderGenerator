...
public class StagedBuilderWithChangedDialogSettingsE2ETest extends BaseBuilderGeneratorIT {
    ...
    public void testWithReversedOrderAndOptional() throws Exception {
        // GIVEN
        ModifiedStagedDialogSettingsAnswerProvider dialogAnswerProvider = new ModifiedStagedDialogSettingsAnswerProvider(Arrays.asList(3, 2, 1, 0),
                Arrays.asList(0, 1, 2));
        given(stagedBuilderStagePropertyInputDialogOpener.open(any(List.class))).willAnswer(dialogAnswerProvider::provideAnswer);
    ...
    public void testWithAllOptionalFields() throws Exception {
        // GIVEN
        ModifiedStagedDialogSettingsAnswerProvider dialogAnswerProvider = new ModifiedStagedDialogSettingsAnswerProvider(asList(0, 1, 2, 3),
                emptyList());
        given(stagedBuilderStagePropertyInputDialogOpener.open(any(List.class))).willAnswer(dialogAnswerProvider::provideAnswer);
        String input = readClasspathFile("multi_field_input.tjava");
