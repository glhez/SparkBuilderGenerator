...
public class BuilderClassRemover implements BuilderRemoverChainItem {
    ...
    public void remove(ASTRewrite rewriter, AbstractTypeDeclaration rootType, CompilationUnitModificationDomain modificationDomain) {
        ...
        if (!generatedAnnotationAnnotatedClasses.isEmpty()) {
    ...
    private void saveCustomMethodsAndRemoveBuilder(ASTRewrite rewriter, AbstractTypeDeclaration clazz,
            CompilationUnitModificationDomain modificationDomain) {
        if (preferencesManager.getBooleanPreference(KEEP_CUSTOM_METHODS_IN_BUILDER)) {
    ...
    private boolean isLikelyACustomMethod(MethodDeclaration method, AbstractTypeDeclaration typeToCreate, AST ast) {
        ...
        if (!(secondStatement instanceof ReturnStatement r && r.getExpression() instanceof ThisExpression)) {
            return true;
        }
        if (!(firstStatement instanceof ExpressionStatement x && x.getExpression() instanceof Assignment)) {
            return true;
        }
    ...
    private long getNumberOfEmptyPrivateConstructors(AbstractTypeDeclaration nestedType) {
