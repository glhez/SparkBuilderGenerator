...
public class SuperConstructorParameterCollector implements FieldCollectorChainItem {
    ...
    public SuperConstructorParameterCollector(FieldNameToBuilderFieldNameConverter fieldNameToBuilderFieldNameConverter,
            PreferencesManager preferencesManager,
            TypeDeclarationFromSuperclassExtractor typeDeclarationFromSuperclassExtractor,
            BodyDeclarationVisibleFromPredicate bodyDeclarationVisibleFromPredicate,
    ...
    public List<? extends BuilderField> collect(AbstractTypeDeclaration typeDeclaration) {
        if (preferencesManager.getBooleanPreference(INCLUDE_PARAMETERS_FROM_SUPERCLASS_CONSTRUCTOR)) {
    ...
    private ConstructorParameterSetterBuilderField createConstructorParameterSetterBuilderField(SingleVariableDeclaration element, int index,
        ...
        Optional<MethodDeclaration> getterMethodDeclaration = bodyDeclarationFinderUtil.findGetterForFieldWithNameAndType(parent, originalFieldName,
                                                                                                                          fieldType);
    ...
    private MethodDeclaration findConstructorToUse(AbstractTypeDeclaration currentType, AbstractTypeDeclaration parentTypeDeclaration) {
        List<MethodDeclaration> applicableConstructors = Arrays.stream(MethodExtractor.getMethods(parentTypeDeclaration))
                                                               ...
                                                               .filter(constructor -> bodyDeclarationVisibleFromPredicate.isDeclarationVisibleFrom(constructor,
                                                                                                                                                   currentType))
                                                               .collect(Collectors.toList());

        if (preferencesManager.getBooleanPreference(PREFER_TO_USE_EMPTY_SUPERCLASS_CONSTRUCTOR)) {
