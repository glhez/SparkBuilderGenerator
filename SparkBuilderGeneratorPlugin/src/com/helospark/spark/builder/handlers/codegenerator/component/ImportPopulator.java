...
public class ImportPopulator {
    ...
    public void populateImports(CompilationUnitModificationDomain compilationUnitModificationDomain) {
        ...
        if (shouldAddNonnullAnnotation(compilationUnit)) {
            String importToAdd = preferencesManager.getBooleanPreference(USE_JAKARTA_ANNOTATION) ? NON_NULL_JAKARTA_FULLY_QUALIFIED_NAME
                    : NON_NULL_FULLY_QUALIFIED_NAME;
            addImport(ast, importRewrite, importToAdd);
        }
        if (shouldAddGeneratedAnnotation(compilationUnit)) {
            String importToAdd = preferencesManager.getBooleanPreference(USE_JAKARTA_ANNOTATION) ? GENERATED_JAKARTA_FULLY_QUALIFIED_NAME
                    : GENERATED_FULLY_QUALIFIED_NAME;
            addImport(ast, importRewrite, importToAdd);
        }
    ...
    private boolean shouldAddNonnullAnnotation(CompilationUnit compilationUnit) {
        return (preferencesManager.getBooleanPreference(ADD_NONNULL_ON_PARAMETERS)
                || preferencesManager.getBooleanPreference(ADD_NONNULL_ON_RETURN)) &&
    ...
    private boolean shouldAddGeneratedAnnotation(CompilationUnit compilationUnit) {
        return preferencesManager.getBooleanPreference(ADD_GENERATED_ANNOTATION)
    ...
    private boolean isImportExists(CompilationUnit compilationUnit, String generatedFullyQualifiedName) {
        return ((List<ImportDeclaration>) compilationUnit.imports())
                                                                    .stream()
                                                                    .filter(importDeclaration -> importDeclaration.getName().toString()
                                                                                                                  .equals(generatedFullyQualifiedName))
                                                                    .findFirst()
